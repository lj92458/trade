...
function headNavBindFunc(){
    //绑定按钮事件
    var firstLevelMenus = $(".nav-main .nav-box").children("li").not(".nav-split,.logo-index");
    firstLevelMenus.each(function(){
        $(this).mouseenter(function(){
            ...
            if($(this).hasClass("selected")){
                ...
                $(this).parent().children("li").not(".nav-split,.logo-index").removeClass("selected");

                $(this).addClass("selected");
            }
            //当前菜单 展开/合起
            var subNavBox = $(this).children(".sub-nav-box");
            if(null != subNavBox && subNavBox.length > 0){
                if(subNavBox.hasClass("hidden")){
                    $(this).parent().children("li").not(".nav-split,.logo-index").children(".sub-nav-box").addClass("hidden");
    ...
    $("html").click(function(event){
        ...
        var navMainObj = navMain.get(0);
        var navLeft = getLeft(navMainObj);
        var navTop = getTop(navMainObj);
        var navWidth = navMainObj.offsetWidth;
        var navHeight = navMainObj.offsetHeight;
        //如果在导航条范围内，忽略此次事件
        if(pageX > navLeft && pageX < navLeft + navWidth && pageY > navTop && pageY < navTop + navHeight){
            return;
        }
        var showingMenus = $(".nav-main .nav-box").children("li").not(".nav-split,.logo-index").children(".sub-nav-box").not(".hidden");
        if(showingMenus.length > 0){
            var showObj = showingMenus.get(0);
            var top = getTop(showObj);
            var left = getLeft(showObj);
            var width = showObj.offsetWidth;
            var height = showObj.offsetHeight;
            if(pageX > left && pageX < left + width && pageY > top && pageY < top + height){
                //在范围内
            }else{
                //范围外
                showingMenus.addClass("hidden");
                showingMenus.parent().removeClass("selected");
            }
        }
    });
}
...
function checkCookies(){
    ...
    if(checkValue == resultValue){
        ...
        //不支持cookies
        AlertMsg("请不要禁用cookie或使用无痕浏览模式，否则您无法体验本站，非常抱歉");
    }
}
