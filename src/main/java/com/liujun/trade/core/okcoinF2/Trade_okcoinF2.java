package com.liujun.trade.core.okcoinF2;
...
public class Trade_okcoinF2 extends Trade {
    ...
    public void flushMarketDeeps() throws Exception {
        ...
        try {
            ...
            //同时存储买单和卖单
            List<List<MarketOrder>> listList = new ArrayList<>(2);
            listList.add(depth.getAskList());
            listList.add(depth.getBidList());
            ...
            for (int j = 0; j < 2; j++) {//为了避免重复的代码。就用一个for循环处理买单和卖单
                ...
                for (int i = 0; i < orderArr.size(); i++) {
                    ...
                    listList.get(j).add(marketOrder);
                }//end for i
    ...
    public int tradeOrder() throws Exception {
        ...
        for (; orderCount < userOrderList.size(); orderCount++) {
            ...
            log.info("下单结果" + result.getError_code() + ":" + result.getError_messsage());
